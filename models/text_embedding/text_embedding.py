...
class AzureOpenAITextEmbeddingModel(_CommonAzureOpenAI, TextEmbeddingModel):
    def _invoke(
        ...
        if not ai_model_entity:
            raise CredentialsValidateFailedError(
                f"Base Model Name {base_model_name} is invalid"
            )
        try:
        ...
        if input_type:
            extra_model_kwargs["input_type"] = (
                input_type.value
                if isinstance(input_type, EmbeddingInputType)
                else input_type
            )
        context_size = int(
            ai_model_entity.entity.model_properties.get(
    ...
    def get_num_tokens(self, model: str, credentials: dict, texts: list[str]) -> list[int]:
        ...
        tokens = []
        try:
            enc = tiktoken.encoding_for_model(self._get_base_model_name(credentials))
        except KeyError:
            enc = tiktoken.get_encoding("cl100k_base")
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        base_model_name = self._get_base_model_name(credentials)
        if not self._get_ai_model_entity(base_model_name, model):
            raise CredentialsValidateFailedError(
                f"Base Model Name {base_model_name} is invalid"
            )
        try:
